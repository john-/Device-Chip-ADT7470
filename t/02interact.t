...
# ->read_duty
{
   $adapter->expect_write_then_read( "\x32", 1 )
      ->returns( "\xFF" );

   is( $chip->read_duty(1)->get, 255,
      '->read_duty result' );
...
# ->read_duty_percent
{
   ...
   is( $chip->read_duty_percent(1)->get, 100,
      '->read_duty_percent result' );
...
# ->write_duty
{
   ...
   is( $chip->write_duty(1, 255)->get, undef,
      '->write_duty result' );
...
# ->write_duty_percent
{
   ...
   is( $chip->write_duty_percent(1, 100)->get, undef,
      '->write_duty_percent result' );
...
# ->_format_fan
throws_ok( sub{ $chip->_format_fan(-1) }, qr/Fan must be 1-4/, 'fan should not be negative' );

# ->_format_fan
throws_ok( sub{ $chip->_format_fan(5) }, qr/Fan must be 1-4/, 'fan should be 1-4' );

# ->_format_fan
throws_ok( sub{ $chip->_format_fan(1.2) }, qr/Fan must be 1-4/, 'fan should be integer' );
